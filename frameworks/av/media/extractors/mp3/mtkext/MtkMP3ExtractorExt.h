...
class MtkMP3SourceExt{
    public:
        MtkMP3SourceExt();
        static int getMultiFrameSize(DataSourceBase *source, off64_t pos,
...
class MtkMP3Toc : public TableOfContentThread{
    public:
        explicit MtkMP3Toc(DataSourceBase *source, uint32_t fixedHeader);
...
int isJointStereoMp3(DataSourceBase *source, off64_t pos,
        uint32_t fixedHeader);

status_t ComputeDurationFromNRandomFrames(DataSourceBase *source, off64_t FirstFramePos,
        uint32_t FixedHeader,int32_t *Averagebr, int32_t *isSingle);

void getGeneralDuration(int bitrate, DataSourceBase *source, MP3Seeker *seeker,
        MetaDataBase *meta, off64_t firstFramePos, uint32_t fixedHeader);
