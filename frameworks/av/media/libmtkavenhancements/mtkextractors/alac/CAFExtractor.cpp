...
namespace android
{
class CAFSource : public MediaTrack, public TableOfContentThread
{
public:
    CAFSource(
...
static bool getCafFormatInfo(
        DataSourceBase *source, off64_t offset, CAFAudioFormat *desc_info, CAFPacketTableHeader *pakt_info,
...
status_t CAFExtractor::getMetaData(MetaDataBase &meta) {
    //TODO
    meta.clear();
...
MediaTrack *CAFExtractor::getTrack(size_t index) {
...
status_t CAFExtractor::getTrackMetaData(MetaDataBase& meta, size_t index, uint32_t /*flags*/) {
    if (mInitCheck != OK || index != 0) {
...
status_t CAFSource::start(MetaDataBase * /*params*/) {
    CHECK(!mStarted);
    ...
    {
        mGroup->add_buffer(MediaBufferBase::Create(mFramesPerPacket * mNumChannels * 4));
    }
    else
    {
        mGroup->add_buffer(MediaBufferBase::Create(mMaxFramesPerPacket * mNumChannels * 2));
    }
    mStarted = true;
...
status_t CAFSource::getFormat(MetaDataBase& meta) {
#ifdef MTK_AOSP_ENHANCEMENT
    int64_t duration;
...
status_t CAFSource::read(
        MediaBufferBase **out, const ReadOptions *options __unused) {
    *out = NULL;
...
status_t CAFSource::stopTocThreadIfTocEnabled() {
    stopTOCThread();
    return NO_ERROR;
}
...
static MediaExtractor* CreateExtractor(
    ...
    return new CAFExtractor(source, offset);
}
