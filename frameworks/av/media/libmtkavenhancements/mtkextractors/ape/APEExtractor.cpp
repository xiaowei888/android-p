...
static bool getAPEInfo(
    DataSourceBase *source, off_t *inout_pos, ape_parser_ctx_t *ape_ctx, bool parseall)
{
    unsigned int i;
...
class APESource : public MediaTrack
{
public:
    APESource(
...
status_t APESource::start(MetaDataBase *)
{
    CHECK(!mStarted);
...
status_t APESource::getFormat(MetaDataBase &meta) {
...
MediaTrack *APEExtractor::getTrack(size_t index)
{
...
status_t APEExtractor::getTrackMetaData(MetaDataBase &meta, size_t index, uint32_t /*flags*/)
{
    if (mInitCheck != OK || index != 0)
    {
...
status_t APEExtractor::getMetaData(MetaDataBase &meta)
{
...
static MediaExtractor* CreateExtractor(
        DataSourceBase *source,
        void *) {
    return new APEExtractor(source);
}
