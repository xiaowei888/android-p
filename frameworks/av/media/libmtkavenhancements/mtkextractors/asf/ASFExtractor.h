...
protected:
    ...
    bool RetrieveWmvCodecSpecificData(asf_stream_t * pStreamProp, MetaDataBase &meta,
            VC1SeqData* pCodecSpecificData);
    bool RetrieveWmaCodecSpecificData(asf_stream_t * pStreamProp, MetaDataBase &meta);
...
public:
    ...
    ASFErrorType GetNextMediaFrame(MediaBufferBase** out, bool& bIsKeyFrame, AsfStreamType strmType,
            bool *isSeeking,uint32_t CurTrackIndex);
    bool ParserVC1CodecPrivateData(uint8_t*input , uint32_t inputlen, VC1SeqData* pSeqData);
    status_t addMPEG4CodecSpecificData(MetaDataBase &mMeta);
    status_t addAVCCodecSpecificData(MetaDataBase &mMeta);
    ...
    status_t asfMakeAVCCodecSpecificData(const sp<ABuffer> &buffer, MetaDataBase &mMeta);
