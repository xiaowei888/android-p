...
class MtkAVISource : public MediaTrack {
    public:
        MtkAVISource(DataSourceBase *dataSource, int index);
...
MtkAVIExtractor::MtkAVIExtractor(DataSourceBase *dataSource)
...
status_t MtkAVIExtractor::getMetaData(MetaDataBase &meta) {
    status_t err;
...
status_t MtkAVIExtractor::getTrackMetaData(
        MetaDataBase &meta,
    ...
    if ((flags & kIncludeExtensiveMetaData) && !track->includes_expensive_metadata) {
        ...
        const char *mime;
        CHECK(track->mFormat.findCString(kKeyMIMEType, &mime));
        if (!strncasecmp("video/", mime, 6)) {
            int64_t dts = (int64_t)track->mThumbNailIndex * 1000000LL
                * track->mScale / track->mRate;
            track->mFormat.setInt64(kKeyThumbnailTime, dts);
...
MediaTrack *MtkAVIExtractor::getTrack(size_t index) {
...
status_t MtkAVIExtractor::parseSTRN(MtkAVIOffT off, MtkAVIOffT end, MtkAVISource* source) {
...

    {
        ...
        source->mFormat.setCString(kKeyMediaLanguage, lang_code);
    }
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, MtkAVISource* source) {
    ...
    char *ptr = NULL;
    MetaDataBase* meta = &(source->mFormat);
    ...
    if (source->mIsVideo) {
        ...
        if (source->mIsDIVX || source->mIsDIVX3) {
            if (!hasDivxLicense()) {
                ALOGW("skip divx(%s) video because of no divx license", mime);
                source->mIsVideo = false;
            }
        }
...
status_t MtkAVIExtractor::parseSTRH(MtkAVIOffT pos, MtkAVIOffT end, MtkAVISource* source) {
    ...
    if (header.fccType == BFOURCC('v','i','d','s')) {
        if (header.rate != 0 && header.scale != 0) {
            source->mFormat.setInt32(kKeyFrameRate, (float)header.rate/header.scale);
        } else {
...
status_t MtkAVIExtractor::checkCapability() {
    for (size_t i = 0; i < mTracks.size(); ++i) {
        ...
        if (source->mIsAudio) {
            meta->setInt32(kKeyMaxInputSize, source->mMaxSampleSize);
...
status_t MtkAVIExtractor::parseFirstRIFF() {
    ...
    if (!mHasIndex) {
        for(size_t i=0; i<mTracks.size(); ++i) {
            ...
            source->mFormat.setInt64(kKeyDuration, time);
...
MtkAVISource::MtkAVISource(DataSourceBase *dataSource, int index)
    : mDataSource(dataSource),
    mIndex(index),
    mIsVideo(false),
    mIsAudio(false),
...
status_t MtkAVISource::getFormat(MetaDataBase &meta) {
    Mutex::Autolock autoLock(mLock);
    ...
    if (mIsAVC) {
        ...
        *out = mBuffer;
        ...
        do{
            ...
            LOGVV("mp3 start %d header %x frameSize %zu length %d bitrate %d",
...
static sp<ABuffer> makeDixv3ESDS(MetaDataBase &format) {
    sp<ABuffer> csd = NULL;
    int32_t width = 0;
    int32_t height = 0;
...
status_t MtkAVISource::generateCodecData(bool full) {
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        ALOGV("kKeyWMAC size is = %d", _config_size);
...
bool MtkSniffAVI(DataSourceBase *source, float *confidence) {
    char header[12];
    ...
    return {
        MediaExtractor::EXTRACTORDEF_VERSION,
            ...
            [](
                ...
                if (MtkSniffAVI(source, confidence)) {
                    return [](
                            DataSourceBase *source,
                            void *) -> MediaExtractor* {
