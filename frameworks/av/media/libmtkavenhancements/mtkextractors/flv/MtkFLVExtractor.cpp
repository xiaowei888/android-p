...
struct FLVSource : public MediaTrack {
...
status_t FLVSource::start(MetaDataBase *params)
{
    int32_t val;
...
status_t FLVSource::getFormat(MetaDataBase &meta)
{
        ...
        if (mExtractor->bHasVideo) {
            if (VIDEO == mType) {
                ...
                if (false == mExtractor->FLVSeekTo((seekTimeUs/1000))) {
                    ALOGE("FLVSeekTo-video failed, and return EOS");
            ...
            mExtractor->ClearAudioFrameQueue();
            if (false == mExtractor->FLVSeekTo((seekTimeUs/1000))) {
                ALOGE("FLVSeekTo-audio failed, and return EOS");
                return ERROR_END_OF_STREAM;
            }
        }
    }
...
status_t FLVExtractor::getMetaData(MetaDataBase &meta) {
 ...
 bool FLVExtractor::ParseFLV()
 {
    ...
    CacheMore(CACHE_FRAME);  // recache by checking bHasVideo bHasAudio
     ...
     MetaDataBase VideoMeta;
     MetaDataBase AudioMeta;
     ...
     if (bHasVideo && VideoMeta.hasData(kKeyMIMEType)) {
     ...
     if (bHasAudio && AudioMeta.hasData(kKeyMIMEType)) {
         mTracks.push();
         TrackInfo *trackInfo = &mTracks.editItemAt(mTracks.size() - 1);
     ...
     else
     {
         for (size_t i = 0; i < mTracks.size(); ++i)
         {
             ...
             for (size_t i = 0; i < mVideoFrames.size(); i++) {
                 ...
                 CHECK(out->meta_data().findInt64(kKeyTime, &_cur_frame_ts));
                 FLV_LOGD("Thumbnail frame TS %lld, Len %zu", (long long)_cur_frame_ts, out->range_length());
...
READ_TAG:
 ...
 MediaBufferBase* FLVExtractor::DequeueVideoFrame(int64_t targetSampleTimeUs)
 {
     ...
     switch(byteVal & 0x0F)
     {
         case FLV_VIDEO_CODEC_ID_SORENSON_SPARK:
         {
             MediaBufferBase *buffer = MediaBufferBase::Create(pTag->tag_header.tag_data_size-FLV_VIDEO_S263_TAG_DATA_RAW_OFFSET);
 ...
 MediaBufferBase* FLVExtractor::DequeueAudioFrame(int64_t targetSampleTimeUs)
 {
     ...
     switch((byteVal & 0xF0)>>4)
     {
         case FLV_AUDIO_CODEC_ID_MP3:
         {
             MediaBufferBase *buffer = MediaBufferBase::Create(pTag->tag_header.tag_data_size-FLV_AUDIO_MP3_TAG_DATA_RAW_OFFSET);
             buffer->meta_data().setInt64(kKeyTime, pTag->tag_header.tag_ts*1000LL);
             uint8_t *data = (uint8_t *)buffer->data();
         ...
         case FLV_AUDIO_CODEC_ID_PCM_LE:
         {
             MediaBufferBase *buffer = MediaBufferBase::Create(pTag->tag_header.tag_data_size-FLV_AUDIO_PCM_TAG_DATA_RAW_OFFSET);
             buffer->meta_data().setInt64(kKeyTime, pTag->tag_header.tag_ts*1000LL);
             uint8_t *data = (uint8_t *)buffer->data();
         ...
         case FLV_AUDIO_CODEC_ID_AAC:
         {
              MediaBufferBase *buffer = MediaBufferBase::Create(pTag->tag_header.tag_data_size-FLV_AUDIO_AAC_TAG_DATA_RAW_OFFSET);
              buffer->meta_data().setInt64(kKeyTime, pTag->tag_header.tag_ts*1000LL);
              uint8_t *data = (uint8_t *)buffer->data();
...
bool SniffFLV(DataSourceBase *source, float *confidence) {
    char header[12];
