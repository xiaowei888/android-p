...
public:
    explicit FLVExtractor(DataSourceBase *source);
    virtual size_t countTracks();
    virtual MediaTrack *getTrack(size_t index);
    virtual status_t getTrackMetaData(MetaDataBase& meta, size_t index, uint32_t flags);
    virtual status_t getMetaData(MetaDataBase& meta);
    FLV_ERROR_TYPE flvMakeHEVCCodecSpecificData(const sp<ABuffer> &buffer, MetaDataBase &mMeta);
...
public:
    ...
    MediaBufferBase* DequeueVideoFrame(int64_t targetSampleTimeUs = -1);
    MediaBufferBase* DequeueAudioFrame(int64_t targetSampleTimeUs = -1);
