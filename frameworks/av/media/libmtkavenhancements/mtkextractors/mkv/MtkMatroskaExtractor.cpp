...
struct MtkDataSourceBaseReader : public mkvparser::IMkvReader {
    explicit MtkDataSourceBaseReader(DataSourceBase *source)
        : mSource(source) {
    }
...
private:
    ...
    MtkDataSourceBaseReader(const MtkDataSourceBaseReader &);
    MtkDataSourceBaseReader &operator=(const MtkDataSourceBaseReader &);
};
...
struct MtkMatroskaSource : public MediaTrack {
    MtkMatroskaSource(MtkMatroskaExtractor *extractor, size_t index);
...
MtkMatroskaSource::MtkMatroskaSource(
        MtkMatroskaExtractor *extractor, size_t index)
        ...
        if (!meta.findData(kKeyAVCC, &dummy, (const void **)&avcc, &avccSize)) {
            ...
            while (!getMetaData) {
                ...
                if (!mPendingFrames.empty()) {
                    MediaBufferBase *buffer = *mPendingFrames.begin();
                    auto accessUnit = heapbuffer<unsigned char>(buffer->range_length());
                    ALOGV("bigbuf->range_length() = %zu", buffer->range_length());
                    memcpy(accessUnit.get(), buffer->data(), buffer->range_length());
                    if (MakeAVCCodecSpecificData(metadata, accessUnit.get(), buffer->range_length())) {
                        getMetaData = true;
                    }
                }
            }
        ...
        if (!meta.findData(kKeyHVCC, &dummy, (const void **)&hvcc, &hvccSize)) {
            ...
            while (!getMetaData) {
                ...
                if (!mPendingFrames.empty()) {
                    ...
                    ALOGV("bigbuf->range_length() = %zu", buffer->range_length());
                    memcpy(accessUnit.get(), buffer->data(), buffer->range_length());
                    if (MakeHEVCCodecSpecificData(metadata, accessUnit.get(), buffer->range_length())) {
                        getMetaData = true;
                    }
                }
            }
...
status_t MtkMatroskaSource::start(MetaDataBase * /* params */) {
...
status_t MtkMatroskaSource::getFormat(MetaDataBase &meta) {
...
status_t MtkMatroskaSource::setWebmBlockCryptoInfo(MediaBufferBase *mbuf) {
...
// add for mtk
    ...
    if (mType != AVC && mType != HEVC) {
        if (MP2_3 == mType) {
            ...
            MediaBufferBase *buffer = MediaBufferBase::Create(frame_size);
...
MtkMatroskaExtractor::MtkMatroskaExtractor(DataSourceBase *source)
    : mDataSource(source),
      mReader(new MtkDataSourceBaseReader(mDataSource)),
      mSegment(NULL),
...
MediaTrack * MtkMatroskaExtractor::getTrack(size_t index) {
    if (index >= mTracks.size()) {
        return NULL;
    }

// add for mtk
    MtkMatroskaSource * matroskasource = new MtkMatroskaSource(this, index);
    ...
    if (mTracks.itemAt(index).mMeta.findInt32(kKeyCodecInfoIsInFirstFrame, &isinfirstframe)
        ...
        matroskasource->setCodecInfoFromFirstFrame();
    }
...
status_t MtkMatroskaExtractor::getTrackMetaData(
        MetaDataBase &meta,
...
// add for mtk, for check if divx file, not support if no divx license
    const mkvparser::SegmentInfo *info = mSegment->GetInfo();
    const char *const muxingApp = info->GetMuxingAppAsUTF8();
// end of add for mtk
...
// add for mtk, for check if divx file, not support if no divx license
        if (track->GetType() == 1 && !hasDivxLicense()) {
            const char *const codecName = track->GetCodecNameAsUTF8();
            if ((codecName != NULL && !strncasecmp("DivX", codecName, 4))
                || (muxingApp != NULL && !strncasecmp("libDivX", muxingApp, 7))) {
                ALOGW("DivX not support as no divx license. muxing app: %s, codec name = %s",
                    muxingApp, codecName);
                continue;
            }
        }
...
status_t MtkMatroskaExtractor::getMetaData(MetaDataBase &meta) {
    meta.setCString(
            kKeyMIMEType,
...
bool MtkSniffMatroska(
        DataSourceBase *source, float *confidence) {
    MtkDataSourceBaseReader reader(source);
    mkvparser::EBMLHeader ebmlHeader;
    long long pos;
...
MediaExtractor::ExtractorDef GETEXTRACTORDEF() {
    return {
        ...
        UUID("7370c7a5-7d17-46e7-863b-b8da1b2857cd"),
        1,
        ...
        [](
            ...
            if (MtkSniffMatroska(source, confidence)) {
                return [](
                        DataSourceBase *source,
                        void *) -> MediaExtractor* {
...
void MtkMatroskaSource::setCodecInfoFromFirstFrame() {
    // use editItemAt not ItemAt, as ItemAt not permit modify mMeta,
    // should use MetaDataBase & type as need to change the data of mMeta.
    MetaDataBase &meta = mExtractor->mTracks.editItemAt(mTrackIndex).mMeta;
    clearPendingFrames();
    int64_t actualFrameTimeUs;
    ...
    if (MPEG4 == mType) {
        ...
        addESDSFromCodecPrivate(
                meta, false, (*mPendingFrames.begin())->data(), vosend);
        ...
        for (size_t i = 0; i < header_lenth; i++)
            ...
            addESDSFromCodecPrivate(meta, false,
        ...
        meta.setInt32(kKeySampleRate, out_sampling_rate);
        meta.setInt32(kKeyChannelCount, out_channels);
    }
