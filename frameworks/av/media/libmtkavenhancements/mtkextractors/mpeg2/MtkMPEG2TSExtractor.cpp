...
struct MtkMPEG2TSSource : public MediaTrack {
    MtkMPEG2TSSource(
            MtkMPEG2TSExtractor *extractor,
            const sp<AnotherPacketSource> &impl,
            bool doesSeek);
    virtual ~MtkMPEG2TSSource();
...
private:
    MtkMPEG2TSExtractor *mExtractor;
...
status_t MtkMPEG2TSSource::start(MetaDataBase *) {
...
status_t MtkMPEG2TSSource::getFormat(MetaDataBase &meta) {
#ifdef MTK_SEEK_AND_DURATION
    if (mImpl == NULL) {
...
MtkMPEG2TSExtractor::MtkMPEG2TSExtractor(DataSourceBase *source)
    : mDataSource(source),
...
MediaTrack *MtkMPEG2TSExtractor::getTrack(size_t index) {
...
status_t MtkMPEG2TSExtractor::getTrackMetaData(
        MetaDataBase &meta,
...
status_t MtkMPEG2TSExtractor::getMetaData(MetaDataBase &meta) {
...
//static
bool MtkMPEG2TSExtractor::isScrambledFormat(MetaDataBase &format) {
    const char *mime;
...
status_t MtkMPEG2TSExtractor::setMediaCas(const uint8_t* casToken, size_t size) {
    HalToken halToken;
...
#ifdef MTK_SEEK_AND_DURATION
    // add for mtk duration calculate method
bool MtkMPEG2TSExtractor::findSyncWord(DataSourceBase *source, off64_t StartOffset,
