...
#include <media/stagefright/MetaDataUtils_MTK.h>
...
// for check divx license
#include <dlfcn.h>
#define DIVX_LICENSE_SO "liblic_divx-sys.so"
#define VAL_NULL (0)
...
bool hasDivxLicense() {
    void *pLicenseHandle, *func;
    VAL_INT32_T (*pfnLicenseBondCheck)(void) = VAL_NULL;
    bool ret = false;
    pLicenseHandle = dlopen(DIVX_LICENSE_SO, RTLD_NOW);
    if (pLicenseHandle == NULL) {
        ALOGW("DivX license so is missed");
        ret = false;
    } else {
        func = dlsym(pLicenseHandle, "fnLicenseBondCheck");
        pfnLicenseBondCheck = (user_reg_scn)(func);
        if (pfnLicenseBondCheck == NULL) {
            ALOGW("DivX license check function is missed");
            ret = false;
        } else {
            if (pfnLicenseBondCheck()) {
                ALOGW("DivX license bond");
                ret = false;
            } else {
                ret = true;
            }
        }
        dlclose(pLicenseHandle);
        pLicenseHandle = NULL;
    }
    return ret;
}
