...
status_t MtkACodec::setupAudioBitWidth(const sp<AMessage> &msg) {
    ...
    if (msg->findInt32("bit-width", &bitWidth)
            && (!strncmp(mComponentName.c_str(), "OMX.MTK.AUDIO.", 14)
                || !strncmp(mComponentName.c_str(), "OMX.google.raw.", 15))) {
...
#ifdef MTK_HIGH_RESOLUTION_AUDIO_SUPPORT
        // for flac: remodify input and output buffer size
        int32_t flacMaxBufferSize;
        if (msg->findInt32("flac-maxBufferSize", &flacMaxBufferSize)) {
            ALOGE("flac-maxBufferSize :%d", (int)flacMaxBufferSize);
            err = setMinBufferSize(kPortIndexInput, (size_t)flacMaxBufferSize);
            if (err != OK) {
                return err;
            }
        }
