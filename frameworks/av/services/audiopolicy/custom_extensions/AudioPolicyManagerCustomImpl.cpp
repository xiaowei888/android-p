...
void gainTableACFXmlChangedCb(AppHandle *appHandle, const char *audioTypeName)
{
    ...
    else if (strcmp(audioTypeName, kFltAudioTypeFileName[AUDIO_COMP_FLT_AUDIO]) == 0) {
        // "PlaybackACF"
    ...
    switch(par1) {
        ...
        case POLICY_LOAD_VOLUME: {
            ...
            for (size_t j = 0; j < mAudioPolicyManager->mOutputs.size(); j++) {
                ...
                audio_devices_t mOutDevices = desc->device();
                for(int i = 0; i < AUDIO_STREAM_CNT; i++) {
                    if (i == AUDIO_STREAM_PATCH) {
                        continue;
                    } else if (desc->isStreamActive((audio_stream_type_t) i)) {
                        // ALOGD("update() mIoHandle %d mId %d device %04x size %zu stream %d", mAudioPolicyManager->mOutputs.keyAt(i), desc->getId(), mOutDevices, mAudioPolicyManager->mOutputs.size(), i);
                        volStatus = mAudioPolicyManager->checkAndSetVolume((audio_stream_type_t)i, mAudioPolicyManager->mVolumeCurves->getVolumeIndex((audio_stream_type_t) i, mOutDevices), desc, mOutDevices, 50, true);
                    }
                }
            }
            break;
...
#if 0//defined(MTK_HIFIVOLUME_SUPPORT)
            ALOGD("POLICY_SET_HIFI_STATE set hifi  volume mode = %d", par2);
            setHIFIVolumeMode(par2);
#endif
...
status_t AudioPolicyManagerCustomImpl::fm_addAudioPatch(audio_patch_handle_t handle, const sp<AudioPatch>& patch)
{
    ...
    if (isFMDirectMode(patch)) {
        if (outputDesc != NULL) {
            ...
            if (status != NO_ERROR) {
                ALOGE("fm_addAudioPatch() cannot active new IO %d", status);
                return status;
            }
    ...
    if (isFMDirectMode(patch)) {
        if (outputDesc != NULL) {
            if (outputDesc->mRefCount[AUDIO_STREAM_MUSIC] > 0) {
                ...
                outputDesc->changeRefCount(AUDIO_STREAM_MUSIC, -1);
                if (outputDesc->mRefCount[AUDIO_STREAM_MUSIC] == 0) {
                    outputDesc->mStopTime[AUDIO_STREAM_MUSIC] = systemTime();
                }
                mFMDirectAudioPatchEnable = false;
...
bool AudioPolicyManagerCustomImpl::lowLatency_setOutputFirstActiveFromStartSource(const sp<AudioOutputDescriptor>& outputDesc, bool beFirstActive, audio_devices_t device)
{
#if defined(MTK_LOW_LATENCY)
    if (beFirstActive) {
        ...
        for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
            ...
            if (desc != outputDesc && outputDesc->sharesHwModuleWith(desc)) {
                if (desc->isActive() && desc->device() != device) {
...
status_t AudioPolicyManagerCustomImpl::offload_invalidateMusicForInCallFromSetPhoneState(audio_mode_t state, int oldState)
{
    if (state == AUDIO_MODE_IN_CALL && oldState != AUDIO_MODE_IN_CALL) {
        for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
            sp<SwAudioOutputDescriptor> desc = mAudioPolicyManager->mOutputs.valueAt(i);
            if (desc->isStreamActive(AUDIO_STREAM_MUSIC) && (desc->mFlags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD)) {
...
status_t AudioPolicyManagerCustomImpl::updatePrimaryModuleDevice(audio_devices_t rxDevice)
{
    ...
    for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
        ...
        if (desc != mAudioPolicyManager->mPrimaryOutput &&
                ...
                (rxDevice != desc->device() || (!mAudioPolicyManager->mPrimaryOutput->isActive() && rxDevice != mAudioPolicyManager->mPrimaryOutput->device()))) {
...
uint32_t AudioPolicyManagerCustomImpl::getSampleRateIndex(uint32_t sampleRate)
{
    ...
    } else if (sampleRate >= OUTPUT_RATE_44_1) {
        return OUTPUT_RATE_44_1_INDEX;
    } else if (sampleRate >= OUTPUT_RATE_32) {
        return OUTPUT_RATE_32_INDEX;
    }else if (sampleRate >= OUTPUT_RATE_22_05) {
        return OUTPUT_RATE_22_05_INDEX;
    }else if (sampleRate >= OUTPUT_RATE_16) {
        return OUTPUT_RATE_16_INDEX;
    }else if (sampleRate >= OUTPUT_RATE_11_025) {
        return OUTPUT_RATE_11_025_INDEX;
    }else if (sampleRate >= OUTPUT_RATE_8) {
        return OUTPUT_RATE_8_INDEX;
    }else {
        return OUTPUT_RATE_48_INDEX;
    }
    return OUTPUT_RATE_48_INDEX;
...
uint32_t AudioPolicyManagerCustomImpl::getMaxTrackSampleRate()
{
#if defined(MTK_HIFIAUDIO_SUPPORT)
    uint32_t SampleRate = OUTPUT_RATE_48;

    if (mAudioPolicyManager->mPrimaryOutput->isStreamActive(AUDIO_STREAM_VOICE_CALL)) {
        //use voice stream sample rate during in call
        SampleRate = mAudioPolicyManager->mPrimaryOutput->mSamplingRate;
...
status_t AudioPolicyManagerCustomImpl::policyForceReplace(audio_stream_type_t stream, uint32_t sampleRate)
{
#if defined(MTK_HIFIAUDIO_SUPPORT)
    // only take place when music stream, FM is an exception for samplerate change
    MTK_ALOGV("%s() stream = %d sampleRate = %d", __FUNCTION__, stream, sampleRate);

    if (stream == AUDIO_STREAM_MUSIC|| stream == AUDIO_STREAM_VOICE_CALL) {
...
status_t AudioPolicyManagerCustomImpl::hifiAudio_startOutputSamplerate(audio_io_handle_t output,
                                                      audio_stream_type_t stream,
                                                      audio_session_t session, int samplerate)
{
#if defined(MTK_HIFIAUDIO_SUPPORT)

    ALOGD("%s() +output = %d +stream = %d session = %d +samplerate = %d mPrimaryOutput = %d HifiState = %d",
        __FUNCTION__, output, stream, session, samplerate, mAudioPolicyManager->mPrimaryOutput->mIoHandle, mHifiState);
    status_t startOutputValue = mAudioPolicyManager->startOutput(output, stream, session);
    ssize_t index = mAudioPolicyManager->mOutputs.indexOfKey(output);
    if (index < 0) {
        ALOGW("%s() unknown output %d", __FUNCTION__, output);
        return BAD_VALUE;
    }
    ...
    if (((bDeepBufferSupported && (desc->mFlags & AUDIO_OUTPUT_FLAG_DEEP_BUFFER)) // if use deep buffer, check for deep buffer flag
        || (!bDeepBufferSupported && (desc->mFlags & AUDIO_OUTPUT_FLAG_PRIMARY)))  // if not use deep buffer, check for non-fast primary  output
        && (startOutputValue == NO_ERROR)) {
...
status_t AudioPolicyManagerCustomImpl::hifiAudio_stopOutputSamplerate(audio_io_handle_t output,
                                                     audio_stream_type_t stream,
                                                     audio_session_t session, int samplerate)
{
#if defined(MTK_HIFIAUDIO_SUPPORT)
    ...
    sp<SwAudioOutputDescriptor> outputDesc = mAudioPolicyManager->mOutputs.valueAt(index);
    bool bDeepBufferSupported = isSupportDeepBuffer();
