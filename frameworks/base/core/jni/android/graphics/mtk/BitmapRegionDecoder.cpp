...
static jobject nativeDecodeRegion(JNIEnv* env, jobject, jlong brdHandle, jint inputX,
	...
	{//MTK
	    ...
	    bool requireUnpremul_MTK = false;		//requireUnpremul
	    jobject tileBitmap = NULL;		//MTK
        ...
        jsize size = 0;
#endif
		bool doDither = true;		//MTK
        bool preferQualityOverSpeed = false;		//MTK

		SkBitmapRegionDecoder_MTK *brd = reinterpret_cast<SkBitmapRegionDecoder_MTK*>(brdHandle);		//MTK
		SkImageDecoder *decoder = brd->getDecoder();		//MTK
		...
		if (NULL != options) {
            ...
            requireUnpremul_MTK = !env->GetBooleanField(options, gOptions_premultipliedFieldID);
		}
        ...
        decoder->setRequireUnpremultipliedColors(requireUnpremul_MTK);
		...
		// add mutex to avoid multi-thread region decode for non-tileBitmap decode mode
		else {
		...
		Bitmap* tmpBitmap = allocator->getStorageObjAndReset();
		// release lock for decoder
		mtkRegionDecodeMutex.release();

		int bitmapCreateFlags = 0;
	    if (!requireUnpremul_MTK) {
	    ...
	    return android::bitmap::createBitmap(env, tmpBitmap, bitmapCreateFlags);
	}
	else
	{//AOSP 
	    // Set default options.
