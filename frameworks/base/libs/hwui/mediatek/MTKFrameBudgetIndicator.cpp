...
static int (*fbcNotifySyncStart)(int, int64_t) = NULL;
...
typedef int (*notify_sync_start)(int, int64_t);
...
static void init()
{
    ...
    func = dlsym(handle, "fbcNotifySyncStart");
    fbcNotifySyncStart = reinterpret_cast<notify_sync_start>(func);

    if (fbcNotifySyncStart == NULL) {
        ALOGE("fbcNotifySyncStart error: %s", dlerror());
        fbcNotifySyncStart = NULL;
        dlclose(handle);
        return;
    }
}
...
int notifySyncStart(int tid, int64_t frame_id)
{
    if (!inited)
        init();

    if (fbcNotifySyncStart) {
        return fbcNotifySyncStart(tid, frame_id);
    }
