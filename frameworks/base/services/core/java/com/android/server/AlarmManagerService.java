...
public class AlarmManagerService extends SystemService {
    ...
    /// M: [ALPS04240118]  Pass NominalTrigger to decide grouping for alarm @{
    protected long getMaxTriggerTimeforAlarmGrouping(int type,long whenElapsed,
       long whenElapsedNominalTrigger, long windowLength, long interval,
       PendingIntent operation, Alarm a){
        return whenElapsed;
    }
    ///@}
    ...
    void reAddAlarmLocked(Alarm a, long nowElapsed, boolean doValidate) {
        ...
        if(supportAlarmGrouping()){
            maxElapsed=getMaxTriggerTimeforAlarmGrouping(a.type,whenElapsed,whenElapsed,
                   a.windowLength, a.repeatInterval, a.operation,a);
        }
        ///@}
    ...
    void setImpl(int type, long triggerAtTime, long windowLength, long interval,
        ...
        final long nowElapsed = SystemClock.elapsedRealtime();
        if (DEBUG_BATCH) {
            Slog.d(TAG, "setImpl debug batch " + operation + ") : type=" + type
        ...
        final long maxElapsed;
        /// M: [ALPS03910104][ALPS04240118] Handled alarm batching and min futurity case @{
        if (supportAlarmGrouping()){
            maxElapsed=getMaxTriggerTimeforAlarmGrouping(type, triggerElapsed, nominalTrigger,
                  windowLength, interval, operation, null);
        /// M: [ALPS03996047] Handled negative windowlength cases @{
            ...
            if (DEBUG_BATCH) {
                if(windowLength < 0) {
                    Slog.wtf(TAG, "Negative window=" + windowLength + " tElapsed=" + triggerElapsed
                            + " maxElapsed=" + maxElapsed + "nominalTrigger: " + nominalTrigger);
                }
            }
        ///@}
        ///@}
