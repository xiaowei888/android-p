...
public class UpstreamNetworkMonitor {
    ...
    private static final boolean VDBG = !android.os.Build.IS_USER;
    ...
    private static TypeStatePair findFirstAvailableUpstreamByType(
        ...
        for (int type : preferredTypes) {
            ...
            for (NetworkState value : netStates) {
                ...
                /// M: [ALPS04103793] Fix AOSP timing issue @{
                /// CONNECTIVITY_CHANGE broadcast may arrive before NetworkCallback.onLost().
                NetworkInfo ni = cm.getNetworkInfo(value.network);
                DetailedState ns = (ni == null) ? null : ni.getDetailedState();
                if (ns == null || ns != DetailedState.CONNECTED) {
                    Log.w(TAG, "Network " + value.network + " is invalid in state " + ns);
                    // just ignore it, it will be cleaned up later via UpstreamNetworkCallback
                    continue;
                }
                /// @}
