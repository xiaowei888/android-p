...
public class MtkWifiDisplayController implements DumpUtils.Dump {
    ...
    private void updateWfdEnableState() {
        ...
        ///M:@{ Waiting for p2p enabled if we enable wifi automatically.
        } else if (mWifiDisplayOnSetting && mAutoEnableWifi) {
            return;
        ///@}
        } else {
    ...
    private void registerEMObserver(int widthPixels, int heightPixels) {
        ...
        WFDCONTROLLER_DISPLAY_TOAST_TIME =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_toast_time"));

        WFDCONTROLLER_DISPLAY_NOTIFICATION_TIME =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_notification_time"));
        WFDCONTROLLER_DISPLAY_RESOLUTION =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_default_resolution"));
        WFDCONTROLLER_DISPLAY_POWER_SAVING_OPTION =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_power_saving_option"));
        WFDCONTROLLER_DISPLAY_POWER_SAVING_DELAY =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_power_saving_delay"));
        WFDCONTROLLER_DISPLAY_SECURE_OPTION =
            mContext.getResources().getInteger(
                getMtkIntegerResourceId("wfd_display_secure_option"));
        ...
        Settings.Global.putInt(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_SQC_INFO_ON"),
                WFDCONTROLLER_SQC_INFO_ON ? 1 : 0);
        Settings.Global.putInt(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_QE_ON"),
                WFDCONTROLLER_QE_ON ? 1 : 0);
        ...
        Settings.Global.putInt(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_AUTO_CHANNEL_SELECTION"),
        ...
        Settings.Global.putInt(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_LATENCY_PROFILING"),
                mLatencyProfiling);
        Settings.Global.putString(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_CHOSEN_CAPABILITY"),
                "");
        ...
        if (SystemProperties.get("ro.vendor.mtk_wfd_support").equals("1")) {
            ...
            mContext.getContentResolver().registerContentObserver(
                    Settings.Global.getUriFor(
                            getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_SECURITY_OPTION")),
                    false, mObserver);
    ...
    private void handleResolutionChange() {
        ...
        doNotRemind = Settings.Global.getInt(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_RESOLUTION_DONOT_REMIND"),
                0) != 0;
    ...
    private void revertResolutionChange() {
        ...
        SystemProperties.set("vendor.media.wfd.video-format", String.valueOf(idxOriginal));

        mResolution = mPrevResolution;

        Settings.Global.putInt(
            mContext.getContentResolver(),
            getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_RESOLUTION"), mResolution);
    }
    ...
    private void showLatencyPanel() {
        ...
        mLatencyPanelView = adbInflater.inflate(getMtkLayoutResourceId("textpanel"), null);

        // text view
        mTextView = (TextView) mLatencyPanelView.findViewById(getMtkIdResourceId("bodyText"));
    ...
    private void resetLatencyInfo() {
        Settings.Global.putString(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_WIFI_INFO"),
                "0,0,0,0");
        Settings.Global.putString(
                mContext.getContentResolver(),
                getMtkSettingsExtGlobalSetting("WIFI_DISPLAY_WFD_LATENCY"),
                "0,0,0");
    }
    ...
    private void clearNotify() {
        ...
        mNotificationManager.cancelAsUser(null,
            getMtkStringResourceId("wifi_display_unstable_connection"), UserHandle.ALL);
    }
        ...
        public void run() {
            ...
            if (mLevel != 0) {
                Toast.makeText(
                    mContext,
                    mResource.getString(
                        getMtkStringResourceId("wifi_display_connection_is_not_steady")),
                    Toast.LENGTH_SHORT).show();
            }
    ...
    private final Runnable mDisplayNotification = new Runnable() {
        ...
        public void run() {
            ...
            if (mLevel != 0) {
                showNotification(
                    getMtkStringResourceId("wifi_display_unstable_connection"),
                    getMtkStringResourceId("wifi_display_unstable_suggestion"));
            }
    ...
    private void showNotification(int titleId, int contentId) {
        ...
        Builder builder = new Notification.Builder(mContext)
                ...
                .setSmallIcon(getMtkDrawableResourceId("ic_notify_wifidisplay_blink"))
                .setAutoCancel(true);
    ...
    private void notifyApDisconnected() {
        ...
        Toast.makeText(
            mContext,
            r.getString(getMtkStringResourceId("wifi_display_wifi_network_disconnected"),
                mWifiApSsid),
            Toast.LENGTH_SHORT).show();

        // Display Notification
        showNotification(
            getMtkStringResourceId("wifi_display_channel_confliction"),
            getMtkStringResourceId("wifi_display_wifi_network_cannot_coexist"));
    }
    ...
    private int getMtkIntegerResourceId(String name) {
        try {
