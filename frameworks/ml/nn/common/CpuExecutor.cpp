...
int CpuExecutor::run(const V1_1::Model& model, const Request& request,
    ...
    for (const auto& operation : model.operations) {
        /// M: For sys trace @{
        ANeuroPilotUtilsPrivate_sysTraceStart(toString(operation.type).c_str());
        /// @}
        ...
        case OperationType::MAX_POOL_2D: {
            ...
            if (input.type == OperandType::TENSOR_FLOAT32) {
                ...
                success = genericPoolingPrepare(input.shape(),
                          ...
                          setInfoAndAllocateIfNeeded(&output, outShape) &&
                          /// M: Fix pooling issue @{
                          /// The output scale might be affected by other tensor, do requantize
                          ...
                          //              reinterpret_cast<uint8_t*>(output.buffer),
                          //              outShape);
                          maxPoolQuant8Requantize(reinterpret_cast<const uint8_t*>(input.buffer),
                                        input.shape(),
                                        padding_left, padding_right,
                                        padding_top, padding_bottom,
                                        stride_width, stride_height,
                                        filter_width, filter_height, activation,
                                        reinterpret_cast<uint8_t*>(output.buffer),
                                        outShape, input.scale, output.scale,
                                        input.zeroPoint, output.zeroPoint);
                          /// M: Fix issue @}
            }
