...
/// M: Fix pooling issue @{
bool maxPoolQuant8Requantize(const uint8_t* inputData, const Shape& inputShape,
                   int32_t padding_left, int32_t padding_right,
                   int32_t padding_top, int32_t padding_bottom,
                   int32_t stride_width, int32_t stride_height,
                   int32_t filter_width, int32_t filter_height, int32_t activation,
    ...
    const int input_offset = -inputZeroPoint;
    const int output_offset = outputZeroPoint;
    const double input_scale = inputScale;
    const double output_scale = outputScale;
    const double real_output_multiplier = input_scale / ((1 << left_shift) * output_scale);
    int32 output_multiplier;
    int output_shift;
    if (!QuantizeMultiplierSmallerThanOne(
            real_output_multiplier, &output_multiplier, &output_shift)) {
        // Get multiplier error, fallback.
        maxPoolQuant8(inputData, inputShape, padding_left, padding_right, padding_top,
                padding_bottom, stride_width, stride_height, filter_width, filter_height,
                activation, outputData, outputShape);
    }

    tflite::optimized_ops::MaxPoolRequantize(left_shift, inputData,
                convertShapeToDims(inputShape), input_offset, output_offset,
