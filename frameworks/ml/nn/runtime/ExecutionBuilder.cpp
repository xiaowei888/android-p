...
static void asyncStartComputePartitioned(const ExecutionBuilder* executionBuilder,
    ...
    while (true) {
        ...
        /// M: Profiler @{
        const_cast<ExecutionBuilder*>(executionBuilder)->setTemporaryData(executor);
        /// @}
...
int StepExecutor::startComputeOnDevice(sp<ExecutionCallback>* synchronizationCallback) {
    ...
    *synchronizationCallback = nullptr;
    ...
    /// M: For sys trace @{
    ANeuroPilotUtilsPrivate_sysTraceStart("PrepareModel");
    /// @}
    ...
    /// M: For sys trace @{
    ANeuroPilotUtilsPrivate_sysTraceStop();
    ANeuroPilotUtilsPrivate_sysTraceStart("Prepare Execution Data");
    /// @}
    ...
    MemoryExt inputPointerArguments;
    MemoryExt outputPointerArguments;
    /// @}
    ...
    /// M: For sys trace @{
    ANeuroPilotUtilsPrivate_sysTraceStop();
    ANeuroPilotUtilsPrivate_sysTraceStart("Execution");
    /// @}
    /// M: Profiler @{
    ANeuroPilotExecutionPrivate_setExecution(std::this_thread::get_id(),
            reinterpret_cast<ANeuralNetworksExecution*>(
            const_cast<ExecutionBuilder*>(this->getExecutionBuilder())));
    int result = ANeuroPilotExecutionPrivate_startProfile(std::this_thread::get_id(), NULL);
    /// @}
    ...
    if (!executeStatus.isOk() || executeStatus != ErrorStatus::NONE) {
        ...
        if (result == ANEURALNETWORKS_NO_ERROR) {
            ANeuroPilotExecutionPrivate_stopProfile(std::this_thread::get_id(),
                    toString(request).c_str(),
                    executeStatus.isOk()
                    ? convertErrorStatusToResultCode(executeStatus)
                    : ANEURALNETWORKS_OP_FAILED);
        }
        ANeuroPilotExecutionPrivate_eraseExecution(std::this_thread::get_id());
        /// @}
    ...
    if (result == ANEURALNETWORKS_NO_ERROR) {
        ANeuroPilotExecutionPrivate_stopProfile(
                std::this_thread::get_id(), toString(request).c_str(), ANEURALNETWORKS_NO_ERROR);
    }
    ANeuroPilotExecutionPrivate_eraseExecution(std::this_thread::get_id());
    /// @}
...
static void asyncStartComputeOnCpu(const Model& model, const Request& request,
    ...
    /// M: Profiler @{
    int result = ANeuroPilotExecutionPrivate_startProfile(std::this_thread::get_id(), "CPU");
    /// @}
    int err = executor.run(model, request, modelPoolInfos, requestPoolInfos);
    /// M: Profiler @{
    if (result == ANEURALNETWORKS_NO_ERROR) {
        ANeuroPilotExecutionPrivate_stopProfile(
                std::this_thread::get_id(), toString(request).c_str(), err);
    }
    ANeuroPilotExecutionPrivate_eraseExecution(std::this_thread::get_id());
    /// @}
...
int StepExecutor::startComputeOnCpu(sp<ExecutionCallback>* synchronizationCallback) {
    ...
    *synchronizationCallback = nullptr;
    ...
    // TODO: Refactor the mechanism.
     /*  The working thread might fast than this thread, that is, the start profiler in
      *  asyncStartComputeOnCpu might be executed earlier than setStepExecutor.
      *  Now, we retry startProfile to avoid this issus, but that will decrease performance.
      *
      *  If the model is moved to asyncStartComputeOnCpu by reference, we can use model as key.
      *  Otherwise, we could only use plugged-in to write a new function asyncStartComputeOnCpuEx
      *  to transfer information.
      */
    ANeuroPilotExecutionPrivate_setExecution(thread.get_id(),
            reinterpret_cast<ANeuralNetworksExecution*>(
            const_cast<ExecutionBuilder*>(this->getExecutionBuilder())));
    /// @}
    executionCallback->bind_thread(std::move(thread));
