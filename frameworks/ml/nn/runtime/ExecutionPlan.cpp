...
public:
    ...
    bool check(size_t operationIndex) const {
        /// M: Fix AOSP Issue @{
        /* The size of mSupportsOperationByIndex might be zero.
        * The getSupportedOperations_1_1 in SampleDriverFloatFast.cpp will
        * return the vector with size is 0 if the model is invaild.
        * Check the vector size to avoid over the boundary of the vector.
        */
        if (operationIndex >= mSupportsOperationByIndex.size()) {
            return false;
        }
        /// @}
