...
int MemoryExt::create(uint32_t size) {
    int ret = ANEURALNETWORKS_NO_ERROR;
    if (ANeuroPilotMemoryPrivate_create(size,
            reinterpret_cast<ANeuralNetworksMemory *>(this)) == ANEURALNETWORKS_NO_ERROR) {
    } else {
...
MemoryExt::~MemoryExt() {
    ANeuroPilotMemoryPrivate_freeMemory(
            reinterpret_cast<ANeuralNetworksMemory *>(this));
}


int MemoryExt::getPointer(uint8_t** buffer) const {
    int ret = ANEURALNETWORKS_NO_ERROR;
    if (ANeuroPilotMemoryPrivate_getPointer(buffer,
            reinterpret_cast<ANeuralNetworksMemory *>(
            const_cast<MemoryExt *>(this))) == ANEURALNETWORKS_NO_ERROR) {
    } else {
        ret = Memory::getPointer(buffer);
    }
    return ret;
}
/// M: Shared Memory Extension @}
