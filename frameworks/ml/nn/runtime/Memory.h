...
public:
    ...
    // once. Copy semantics could be enabled if some sort of reference counting
    // or deep-copy system for runtime objects is added later.
    MemoryExt(const MemoryExt&) = delete;
    MemoryExt& operator=(const MemoryExt&) = delete;

    /// M: Extended Functions
    int create(uint32_t size) override;

    int getPointer(uint8_t** buffer) const override;

    /// M: Added functions
    void setHidlMemory(hardware::hidl_memory hidlMemory) { mHidlMemory = hidlMemory; }
    void setMemoryType(int type) { mMemoryType = type; }
...
private:
    /// M: Added variable
    int mMemoryType = ANEUROPILOT_MEMORY_ASHMEM;
};
/// M: Shared Memory Extension  @}
