...
void TimeStats::removeTimeRecord(const std::string& layerName, uint64_t frameNumber) {
    ...
    // When the waitDate points the last item and try to remove it, AOSP does not decrease
    // the waitData. Then TimeStats may access over boundary of layer record. Therfore we
    // check the waitData which is valid or not in here. If the waitData is over the size
    // of layer record, we has to reset the waitData to the last item.
    if (layerRecord.waitData < 0 ||
        layerRecord.waitData >= static_cast<int32_t>(layerRecord.timeRecords.size()))
        layerRecord.waitData = layerRecord.timeRecords.size() - 1;
#endif
