...
public class MtkWapi {
        ...
        synchronized (getLockForSupplicantStaIfaceHal()) {
            ...
            for (ISupplicant.IfaceInfo ifaceInfo : supplicantIfaces) {
                if (ifaceInfo.type == IfaceType.STA && ifaceName.equals(ifaceInfo.name)) {
                    try {
                        getMtkSupplicantMockableV2_0().getInterface(ifaceInfo,
    ...
    private static String getInterfaceName() {
        ...
        try {
            Field field = wifiStateMachine.getClass().getDeclaredField("mInterfaceName");
            field.setAccessible(true);
            return (String) field.get(wifiStateMachine);
        } catch (ReflectiveOperationException e) {
            e.printStackTrace();
            return "";
        }
