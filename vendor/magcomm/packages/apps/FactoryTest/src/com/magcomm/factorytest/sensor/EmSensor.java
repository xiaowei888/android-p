...
import com.magcomm.factorytest.emsvr.AFMFunctionCallEx;
import com.magcomm.factorytest.emsvr.FunctionReturn;
...
public class EmSensor {
    ...
    public static int startGsensorCalibration() {
        String[] ret = runCmdInEmSvr(
                AFMFunctionCallEx.FUNCTION_EM_SENSOR_START_GSENSOR_CALIBRATION, 0);
        if (ret.length > 0 && String.valueOf(RET_STATIC_CALI_SUCCESS).equals(ret[0])) {
            return RET_STATIC_CALI_SUCCESS;
        }
        return RET_STATIC_CALI_FAILED;
    }

    public static int getGsensorStaticCalibration(float[] result) {
        String[] ret = runCmdInEmSvr(
                AFMFunctionCallEx.FUNCTION_EM_SENSOR_GET_GSENSOR_STATIC_CALIBRATION, 0);
        if (ret.length >= 4 && String.valueOf(RET_STATIC_CALI_SUCCESS).equals(ret[0])) {
            try {
                result[0] = Float.parseFloat(ret[1]);
                result[1] = Float.parseFloat(ret[2]);
                result[2] = Float.parseFloat(ret[3]);
    ...
    public static int startGyroscopeCalibration() {
        String[] ret = runCmdInEmSvr(
                AFMFunctionCallEx.FUNCTION_EM_SENSOR_START_GYROSCOPE_CALIBRATION,0);
        if (ret.length > 0 && String.valueOf(RET_STATIC_CALI_SUCCESS).equals(ret[0])) {
            return RET_STATIC_CALI_SUCCESS;
        }
        return RET_STATIC_CALI_FAILED;
    ...
    public static int getGyroscopeStaticCalibration(float[] result) {
        String[] ret = runCmdInEmSvr(
                AFMFunctionCallEx.FUNCTION_EM_SENSOR_GET_GYROSCOPE_STATIC_CALIBRATION, 0);
        if (ret.length >= 4 && String.valueOf(RET_STATIC_CALI_SUCCESS).equals(ret[0])) {
            try {
                result[0] = Float.parseFloat(ret[1]);
                result[1] = Float.parseFloat(ret[2]);
    ...
    static {
        System.loadLibrary("emsensorjni");
    }
}
